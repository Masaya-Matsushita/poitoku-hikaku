...
4. 表の「切れると止まるもの」を警告に添える。現在の構成では `SUPABASE_ACCESS_TOKEN` は CI で未使用なので期限切れで止まるものは無く、日次クロール（`crawl.yml`、`SUPABASE_SECRET_KEY`）もマイグレーション適用（`SUPABASE_DB_PASSWORD`、無期限）も影響を受けない旨を書く。オーナーが緊急度を判断できるようにするため
